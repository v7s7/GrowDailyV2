...
//
// The rule: a day enters a score as soon as a member records real work, or
// once it is CLOSED (kDayCutoffHour the next morning). A blank open day stays
// outside both sides, so a new day never penalizes anyone; a partial day is
// visible immediately instead of making the room total look stuck.
...
void main() {
  group('dayIsCountableAt', () {
    test('a blank unfinished day is not countable while it is still open', () {
      final hoor = _hoor();
      // Use a blank member: work recorded on an open day now belongs in the
      // live total, but a blank day remains outside it.
      final blank = _member(name: 'blank', done: const {});
      expect(blank.dayIsCountableAt('2026-09-11', _beforeMidnight), isFalse);
      expect(blank.dayIsCountableAt('2026-09-11', _afterMidnight), isFalse);
      // It closes at 10:00 the next morning, and counts from then.
      expect(hoor.dayIsCountableAt('2026-09-11', _afterCutoff), isTrue);
    });

    test('a partial open day counts as soon as work is recorded', () {
      final hoor = _hoor();
      // Her 11th is two of three. The live room total must show that work
      // immediately, not wait for the final habit or tomorrow's cutoff.
      expect(hoor.dayIsCountableAt('2026-09-11', _beforeMidnight), isTrue);
      expect(hoor.dayIsCountableAt('2026-09-11', _afterMidnight), isTrue);
    });
  ...
  group('no midnight dip', () {
    ...
    test('Hoor reads 70%, not the 64% the midnight reading gave her', () {
      // 7 2/3 of 11 live days: 0.5 + 0.5 on the 1st and 2nd, nothing on the
      // 3rd and 4th, then six whole days, plus two of three on the 11th.
      // The 12th has not started, so it stays outside the fraction.
      final hoor = _hoor();
      final room = _room();
      expect(_percent(hoor.progressRatio(room, now: _afterMidnight)), 70);
      expect(
        hoor.daysCompleted(room, now: _afterMidnight),
        closeTo(7 + 2 / 3, 1e-9),
      );
      expect(hoor.daysElapsedIn(room, now: _afterMidnight), 11);
    });
  ...
  group('the room score shows live progress', () {
    test('a partly finished open day immediately enters the room score', () {
      final room = _room();
      final hoor = _hoor();
      expect(hoor.roomDaysElapsedIn(room, now: _afterMidnight), 11);
      expect(
