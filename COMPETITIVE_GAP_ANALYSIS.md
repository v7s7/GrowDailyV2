...
> **Status, September 2026.** Shipped since this was written: the Tasbih counter (#1), the Apple Health / Health Connect steps link (#4), a mood check-in inside Night Review (#7), and the Android build (#9). A prayer-time focus / app-blocking mode was built and then deliberately removed from the app; it is not a gap and should not be proposed again, so its section is gone from this document.
...
### 1. No dedicated Dhikr/Tasbih counter
...
### 5. Rooms is competitive, not cooperative
...
### 7. No mood/wellness signal over time
...
### 9. Platform reach (brief, since this is strategy more than a feature)
...
Roughly in order of impact-for-effort based on the research above: the Ramadan mode (#2) is seasonal content on top of infrastructure that already exists — the cheapest real win here. Data export (#8) is low effort and a pure trust win. The weekly digest (#6) reuses Insights' existing computation and turns a pull feature into a push one. AI coaching (#3) is the one genuinely bigger lift — worth planning for, not squeezing in alongside the others. The Tasbih counter (#1), the Apple Health link (#4), the mood check-in (#7) and the Android build (#9) have shipped since this was written.
